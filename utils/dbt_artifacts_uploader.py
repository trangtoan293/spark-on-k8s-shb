...
def _upload_via_boto3(paths: List[Path], bucket: str, prefix: str) -> bool:
    try:
...
def _upload_via_aws_cli(paths: List[Path], bucket: str, prefix: str) -> bool:
    ok = True
    for p in paths:
        key = f"{prefix.rstrip('/')}/{p.name}" if prefix else p.name
...
def upload_dbt_artifacts(
    ...
    # Try boto3 first
    if _upload_via_boto3(artifacts, bucket=bucket, prefix=prefix):
        return True

    # Fallback to AWS CLI
    return _upload_via_aws_cli(artifacts, bucket=bucket, prefix=prefix)
